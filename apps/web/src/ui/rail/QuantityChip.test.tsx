...
describe('the chip', () => {
  it('shows the quantity that is active', () => {
    render(
      <QuantityChip labelledBy="setting-name" quantity={100} onChange={() => {}} copy={copy} />,
    );
  ...
  it('advances on a press', async () => {
    ...
    render(
      <QuantityChip labelledBy="setting-name" quantity={10} onChange={onChange} copy={copy} />,
    );
  ...
  it('says which quantity is set', () => {
    render(
      <>
        <span id="setting-name">{copy.quantity}</span>
        <QuantityChip labelledBy="setting-name" quantity={10} onChange={() => {}} copy={copy} />
      </>,
    );
    expect(screen.getByRole('button')).toHaveAccessibleName(/Buy 10 at a time/);
  });

  it("takes the strip's printed label rather than repeating its words", () => {
    render(
      <>
        <span id="setting-name">{copy.quantity}</span>
        <QuantityChip labelledBy="setting-name" quantity={10} onChange={() => {}} copy={copy} />
    ...
    expect(screen.getByRole('button')).toHaveAccessibleName(
      `${copy.quantity} ${copy.quantityOption('10')}`,
    );
  });
