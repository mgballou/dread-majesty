...
describe('ChainLink', () => {
  it('carries nothing while the producer has produced nothing', () => {
    const { container } = render(<ChainLink produced={new Decimal(0)} version={0} tone="tier-2" />);
  ...
  it('pours motes down the link when a cycle completes', () => {
    const { container, rerender } = render(
      <ChainLink produced={new Decimal(0)} version={0} tone="tier-2" />,
    );

    rerender(<ChainLink produced={new Decimal(1500)} version={1} tone="tier-2" />);
  ...
  it('sets the head deepest, so it leads whichever way the string is drawn', () => {
    const { container, rerender } = render(
      <ChainLink produced={new Decimal(0)} version={0} tone="tier-2" />,
    );
    ...
    expect(container.querySelector('.stage-link__mote--head')).toHaveStyle({
      top: 'calc(var(--stage-link-spread) * 1)',
    });
  });
  ...
  it('draws as many motes as the amount is worth', () => {
    ...
    rerender(<ChainLink produced={new Decimal('1e40')} version={1} tone="tier-2" />);
  ...
  it('stays still while the producer stands idle', () => {
    ...
    const { container, rerender } = render(
      <ChainLink produced={produced} version={0} tone="tier-2" />,
    );

    rerender(<ChainLink produced={produced} version={1} tone="tier-2" />);

    expect(motes(container)).toHaveLength(0);
  });

  it('takes its tone from the manifest, never the accent', () => {
    const { container } = render(<ChainLink produced={new Decimal(0)} version={0} tone="tier-3" />);
